...
public class MaxCounter {
	...
	public int[] solution(int N, int[] A) {
		...
		int[] needToReturn = new int[N];
		int maxCount = 0;
		int minCount = 0;
		for (int i = 0; i < A.length; i++) {
			if (A[i] <= N) {
				// If there is any elements that greater than N occurs then
				// increase the number by one
				if (needToReturn[A[i] - 1] < minCount) {
					needToReturn[A[i] - 1] = minCount;
				}
				needToReturn[A[i] - 1]++;
				if (maxCount < needToReturn[A[i] - 1]) {
					maxCount = needToReturn[A[i] - 1];
				}
			} else {
				minCount = maxCount;
			}
		}
		// Merge Mincount to All the elements. which is missed in iteration
		for (int i = 0; i < needToReturn.length; i++) {
			if (needToReturn[i] < minCount) {
				needToReturn[i] = minCount;
			}
		}
		return needToReturn;
	}
